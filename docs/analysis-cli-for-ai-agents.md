...
| 우선순위 | 개선사항 | 노력 | 효과 | 비고 |
|---------|---------|------|------|------|
| 해당 없음 | `CLAUDE.md` 에이전트 가이드 추가 | 낮음 | 개발 세션에만 유효 | CLI를 사용하는 외부 세션에서는 읽히지 않음. 사용처 프로젝트의 CLAUDE.md에 기술하거나 --help에 의존 |
...
- **CLAUDE.md** — 이 프로젝트 개발 세션에서만 읽힘. CLI를 사용하는 외부 프로젝트 세션에서는 효과 없음. 사용처 프로젝트의 CLAUDE.md에 CLI 사용법을 기술하거나, `--help`의 자기 복구 루프에 의존하는 것이 현실적.
...
| 런타임 스키마 인트로스펙션 | 필수 — 수백 개 커맨드를 docs로 커버 불가 | 선택적 — `--help` + 에러 메시지 루프로 충분 |
| 동적 커맨드 서페이스 | 핵심 — API Discovery 기반 자동 생성 | 해당 없음 — Linear SDK는 정적 |
| NDJSON 페이지네이션 | 유용 — 대량 데이터 스트리밍 | 과잉 — limit 10~50건이면 단일 JSON 배열로 충분 |
| Agent Skills 문서 | 필수 — 100+ SKILL.md | 사용처 프로젝트의 CLAUDE.md에 기술하거나 `--help`에 의존 |
...
다만 `gws`와의 규모 차이를 인식해야 한다. 이 CLI는 바이너리로 빌드되어 외부 프로젝트에서 사용되므로, 이 프로젝트의 `CLAUDE.md`는 CLI 사용 세션에서는 읽히지 않는다. 따라서 CLI 자체가 자기 기술적이어야 하며, 현재의 `--help` 텍스트 + 에러 메시지 안내 패턴이 그 역할을 수행하고 있다. `--help --json`이나 `--dry-run`은 프로젝트가 성장하여 커맨드 수가 늘어날 때 재검토하면 된다.
